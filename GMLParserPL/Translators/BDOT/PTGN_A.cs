...
{
    internal class PTGN_A : AreaToPointsTranslator
    {
        public PTGN_A(string bdotClass, string filePath, Config config) : base(bdotClass, filePath, config)
        {
        ...
        {
            if (config.PTGN_A_IIPObj.ContainsKey(objectAsDict["idIIP"].ToString()))
            {
                return config.PTGN_A_IIPObj[objectAsDict["idIIP"].ToString()];
            }

            if (config.PTGN_A_Obj.ContainsKey(objectAsDict["x_kod"].ToString()))
            {
                return config.PTGN_A_Obj[objectAsDict["x_kod"].ToString()];
            }
            return null;
        }
