...
{
    internal class PTTR_A : AreaToPointsTranslator
    {
        public PTTR_A(string bdotClass, string filePath, Config config) : base(bdotClass, filePath, config)
        {
        ...
        {
            if (config.PTTR_A_IIPObj.ContainsKey(objectAsDict["idIIP"].ToString()))
            {
                return config.PTTR_A_IIPObj[objectAsDict["idIIP"].ToString()];
            }

            if (config.PTTR_A_Obj.ContainsKey(objectAsDict["x_kod"].ToString()))
            {
                return config.PTTR_A_Obj[objectAsDict["x_kod"].ToString()];
            }
            return null;
        }
