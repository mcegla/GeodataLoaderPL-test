...
{
    internal class PTWZ_A : AreaToPointsTranslator
    {
        public PTWZ_A(string bdotClass, string filePath, Config config) : base(bdotClass, filePath, config)
        {
        ...
        {
            if (config.PTWZ_A_IIPObj.ContainsKey(objectAsDict["idIIP"].ToString()))
            {
                return config.PTWZ_A_IIPObj[objectAsDict["idIIP"].ToString()];
            }

            if (config.PTWZ_A_Obj.ContainsKey(objectAsDict["x_kod"].ToString()))
            {
                return config.PTWZ_A_Obj[objectAsDict["x_kod"].ToString()];
            }
            return null;
        }
