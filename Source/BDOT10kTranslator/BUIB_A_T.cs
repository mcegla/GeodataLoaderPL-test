...
{
    //===================================================================================
    //=== Klasa odpowiedzialna za tłumaczenie klasy BDOT10k - BUIB_A na obiekty Unity ===
    //-----------------------------------------------------------------------------------
    //====== Class responsible for translating BDOT10k - BUIB_A class to Unity obj ======
    //===================================================================================
    //http://prawo.sejm.gov.pl/isap.nsf/download.xsp/WDU20112791642/O/D20111642-02.pdf
    class BUIB_A_T
    {
        public void BUIB_A(GeodataLoaderConfiguration config)
        {
            var type = "BUIB_A"; // końcówka nazwy pliku / end of file name
            string file = null;
            try
            {
            ...
            var parser = new Parser_BUIB_A(); // wykorzystaj ten parser / use this parser
            ...
            foreach (var entity in parser.GetBDOT10Ks()) // (gml featuremember)
            {
                ...
                if (BUIB_A_Dic.BuildingXkodDic.ContainsKey(entity.XKod)) // jeżeli xkod istnieje w danym słowniku / if xkod exists in dictionary
                    BuildingFactory.Create(avgPoint.x, avgPoint.y, angle, BUIB_A_Dic.BuildingXkodDic[entity.XKod]); // stwórz obiekt odpowiedniego typu / create object of specified type 
                else
