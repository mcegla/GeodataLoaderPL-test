...
{
    //===================================================================================
    //=== Klasa odpowiedzialna za tłumaczenie klasy BDOT10k - BUIT_P na obiekty Unity ===
    //-----------------------------------------------------------------------------------
    //====== Class responsible for translating BDOT10k - BUIT_P class to Unity obj ======
    //===================================================================================
    //http://prawo.sejm.gov.pl/isap.nsf/download.xsp/WDU20112791642/O/D20111642-02.pdf
    class BUIT_P_T
    {
        public void BUIT_P(GeodataLoaderConfiguration config)
        {
            var type = "BUIT_P"; // końcówka nazwy pliku / end of file name
            string file = null;
            try
            {
            ...
            var parser = new Parser_BUIT_P(); // wykorzystaj ten parser / use this parser
            ...
            foreach (var entity in parser.GetBDOT10Ks()) // (gml featuremember)
            {
                ...
                if (CoordinatesCalculator.IsInRange(p)) // jeżeli w obszarze gry / if in range of game area
                {
                    ...
                    if (BUIT_P_Dic.PropXkodDic.ContainsKey(entity.XKod)) // jeżeli xkod istnieje w danym słowniku / if xkod exists in dictionary
                        PropFactory.Create(p.x, p.y, angle, BUIT_P_Dic.PropXkodDic[entity.XKod]); // stwórz obiekt odpowiedniego typu / create object od specified type 
                    else if (BUIT_P_Dic.BuildingXkodDic.ContainsKey(entity.XKod))
                        BuildingFactory.Create(p.x, p.y, angle, BUIT_P_Dic.BuildingXkodDic[entity.XKod]);
                }
            }
        }
    }
