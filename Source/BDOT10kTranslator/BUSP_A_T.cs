...
{
    //===================================================================================
    //=== Klasa odpowiedzialna za tłumaczenie klasy BDOT10k - BUSP_A na obiekty Unity ===
    //-----------------------------------------------------------------------------------
    //====== Class responsible for translating BDOT10k - BUSP_A class to Unity obj ======
    //===================================================================================
    //http://prawo.sejm.gov.pl/isap.nsf/download.xsp/WDU20112791642/O/D20111642-02.pdf
    class BUSP_A_T
    {
        public void BUSP_A(GeodataLoaderConfiguration config)
        {
            var type = "BUSP_A"; // końcówka nazwy pliku / end of file name
            string file = null;
            try
            {
            ...
            var parser = new Parser_BUSP_A(); // wykorzystaj ten parser / use this parser
            ...
            foreach (var entity in parser.GetBDOT10Ks()) // (gml featuremember)
            {
                ...
                if (BUSP_A_Dic.BuildingXkodDic.ContainsKey(entity.XKod)) // jeżeli fun istnieje w danym słowniku / if fun exists in dictionary
                    BuildingFactory.Create(avgPoint.x, avgPoint.y, angle, BUSP_A_Dic.BuildingXkodDic[entity.XKod]); // stwórz obiekt odpowiedniego typu / create object of specified type 
                else
