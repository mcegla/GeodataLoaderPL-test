...
{
    //===================================================================================
    //=== Klasa odpowiedzialna za tłumaczenie klasy BDOT10k - OIKM_P na obiekty Unity ===
    //-----------------------------------------------------------------------------------
    //====== Class responsible for translating BDOT10k - OIKM_P class to Unity obj ======
    //===================================================================================
    //http://prawo.sejm.gov.pl/isap.nsf/download.xsp/WDU20112791642/O/D20111642-02.pdf
    class OIKM_P_T
    {
        public void OIKM_P(GeodataLoaderConfiguration config)
        {
            var type = "OIKM_P"; // końcówka nazwy pliku / end of file name
            string file = null;
            try
            {
            ...
            var parser = new Parser_OIKM_P(); // wykorzystaj ten parser / use this parser
            ...
            foreach (var entity in parser.GetBDOT10Ks()) // (gml featuremember)
            {
                ...
                if (CoordinatesCalculator.IsInRange(p)) // jeżeli w obszarze gry / if in range of game area
                {
                    ...
                    if (OIKM_P_Dic.PropXkodDic.ContainsKey(entity.XKod)) // jeżeli xkod istnieje w danym słowniku / if xkod exists in dictionary
                        PropFactory.Create(p.x, p.y, angle, OIKM_P_Dic.PropXkodDic[entity.XKod]); // stwórz obiekt odpowiedniego typu / create object od specified type 
                    else if (OIKM_P_Dic.BuildingXkodDic.ContainsKey(entity.XKod))
                        BuildingFactory.Create(p.x, p.y, angle, OIKM_P_Dic.BuildingXkodDic[entity.XKod]);
                }
            }
        }
    }
