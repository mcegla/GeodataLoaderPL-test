...
{
    //===================================================================================
    //=== Klasa odpowiedzialna za tłumaczenie klasy BDOT10k - OISZ_A na obiekty Unity ===
    //-----------------------------------------------------------------------------------
    //====== Class responsible for translating BDOT10k - OISZ_A class to Unity obj ======
    //===================================================================================
    //http://prawo.sejm.gov.pl/isap.nsf/download.xsp/WDU20112791642/O/D20111642-02.pdf
    class OISZ_A_T
    {
        public void OISZ_A(GeodataLoaderConfiguration config)
        {
            var type = "OISZ_A"; // końcówka nazwy pliku / end of file name
            string file = null;
            try
            {
            ...
            var parser = new Parser_OISZ_A(); // wykorzystaj ten parser / use this parser
            ...
            foreach (var entity in parser.GetBDOT10Ks()) // (gml featuremember)
            {
                ...
                var points = PointInPoly.CreatePointArray(minMax[0], minMax[1], 10);
                ...
                foreach (var p in points) // sprawdź czy każdy ze stworzonych punktów jest wewnątrz poligonu / for each point check if it lies inside of polygon  
                {
                        ...
                        && (interiors == null || !interiors.Any(interior => PointInPoly.pnpoly(interior, p.x, p.y))))
                    {
                        //try
                        //{
                        //    // spróbuj stworzyć obiekt  / try creating object
                        //    TreeFactory.Create(p.x, p.y, "");
                        //}
                        //catch
                        //{
                            ...
                            CommonHelpers.Log($"Couldn't create OISZ01");
                        //}
                    }
                }
            }
