...
{
    //===================================================================================
    //=== Klasa odpowiedzialna za tłumaczenie klasy BDOT10k - PTRK_A na obiekty Unity ===
    //-----------------------------------------------------------------------------------
    //====== Class responsible for translating BDOT10k - PTRK_A class to Unity obj ======
    //===================================================================================
    //http://prawo.sejm.gov.pl/isap.nsf/download.xsp/WDU20112791642/O/D20111642-02.pdf
    class PTRK_A_T
    {
        public void PTRK_A(GeodataLoaderConfiguration config)
        {
            var type = "PTRK_A"; // końcówka nazwy pliku / end of file name
            string file = null;
            try
            {
            ...
            var parser = new Parser_PTRK_A(); // wykorzystaj ten parser / use this parser
            ...
            foreach (var entity in parser.GetBDOT10Ks()) // (gml featuremember)
            {
                ...
                var points = PointInPoly.CreatePointArray(minMax[0], minMax[1], 15);
                ...
                foreach (var p in points) // sprawdź czy każdy ze stworzonych punktów jest wewnątrz poligonu / for each point check if it lies inside of polygon  
                {
                        ...
                        && (interiors == null || !interiors.Any(interior => PointInPoly.pnpoly(interior, p.x, p.y))))
                    {
                        try
                        {
                            ...
                            TreeFactory.Create(p.x, p.y, PTRK_A_Dic.TreeXkodDic[entity.XKod]);
                        }
                        catch
                        {
