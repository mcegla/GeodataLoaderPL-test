...
{
    //===================================================================================
    //=== Klasa odpowiedzialna za tłumaczenie klasy BDOT10k - PTWP_A na obiekty Unity ===
    //-----------------------------------------------------------------------------------
    //====== Class responsible for translating BDOT10k - PTWP_A class to Unity obj ======
    //===================================================================================
    //http://prawo.sejm.gov.pl/isap.nsf/download.xsp/WDU20112791642/O/D20111642-02.pdf
    class PTWP_A_T
    {
        public void PTWP_A(GeodataLoaderConfiguration config)
        {
            var type = "PTWP_A"; // końcówka nazwy pliku / end of file name
            string file = null;
            try
            {
            ...
            var parser = new Parser_PTWP_A(); // wykorzystaj ten parser / use this parser
            ...
            foreach (var entity in parser.GetBDOT10Ks()) // (gml featuremember)
            {
                ...
                var points = PointInPoly.CreatePointArray(minMax[0], minMax[1], 25); // prawdopodobnie wystarczająca / probably enough
                ...
                foreach (var p in points) // sprawdź czy każdy ze stworzonych punktów jest wewnątrz poligonu / for each point check if it lies inside of polygon  
                {
                        ...
                        && (interiors == null || !interiors.Any(interior => PointInPoly.pnpoly(interior, p.x, p.y))))
                    {
                        try
                        {
                            ...
                            new WaterFactory().newWaterSource(p.x, p.y, 3, 2, 5); // prawdopodobnie wystarczająco / probably enough
                        }
                        catch
                        {
